...
public class SimpleIdentifiableRepresentationModelAssembler<T> implements
	...
	public void addLinks(EntityModel<T> resource) {
		...
		resource.add(getItemLinkBuilder().slash(getId(resource)).withSelfRel());
		resource.add(getItemLinkBuilder().withRel(this.relProvider.getCollectionResourceRelFor(this.resourceType)));
	}
	...
	protected LinkBuilder getItemLinkBuilder() {

		WebMvcLinkBuilder linkBuilder = WebMvcLinkBuilder.linkTo(this.controllerClass);

		for (String pathComponent : (getPrefix() + this.relProvider.getItemResourceRelFor(this.resourceType))
				.split("/")) {
			if (!pathComponent.isEmpty()) {
				linkBuilder = linkBuilder.slash(pathComponent);
			}
